...
namespace App\Filament\Reception\Resources\Reservations\Tables;
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('chambre.type')
                    ->label('Type')
                    ->badge(),
                TextColumn::make('chambre.statut')
                    ->label('État chambre')
                    ->badge()
            ...
            ->recordActions([
                Action::make('checkin')
                    ...
                    ->action(function ($record) {
                        if ($record->chambre->statut !== 'disponible') {
                            Notification::make()
                                ->title('Check-in impossible')
                                ->body('La chambre ' . $record->chambre->numero . ' est en statut "' . $record->chambre->statut . '". La gouvernante doit d\'abord la marquer comme disponible.')
                                ->danger()
                                ->send();
                            return;
                        }
